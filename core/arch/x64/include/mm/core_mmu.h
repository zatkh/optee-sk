...
#define AREA_MAPPED_W_ONE_PT  (2 * MB) // 512 * 4 * 2^10

#define NO_OF_PML4_ENTRIES    1
#define NO_OF_PDP_ENTRIES     512
#define NO_OF_PD_ENTRIES      512
#define NO_OF_PT_ENTRIES      512
#define NO_OF_PT_TABLES       40 // why did we choose only 2 page table in cpu.c
...
struct mmu_initial_mapping { //new 
	paddr_t phys;
	vaddr_t virt;
	size_t size;
	unsigned int flags;
 ...
 */
/* TODO: If we want to enable virtualization, change definition to 
 * 'bool core_mmu_find_table(struct mmu_partition prtn, vaddr_t va, 
 * unsigned max_level, struct core_mmu_table_info *tbl_info);' 
 * and add in functionality from core_mmu_lpae.c 
 */
...
#endif
// new through "#endif __ASSEMBLER__"
