...
#ifdef X64
static bool is_from_user(void)
{
	// TODO: Figure this out for x64.
	return false;
}
static bool is_user_mode(struct thread_ctx_regs *regs)
{
			    ...
			    uint32_t a2, uint32_t a3)
{
	...
	thread_resume(&threads[n].regs);
#elif defined(X64)
	thread_rpc_resume(threads[n].stack_va_curr[threads[n].ta_idx], a0, a1, a2,
		a3);
#endif
	/*NOTREACHED*/
	panic();
...
{
#if !defined(CFG_WITH_ARM_TRUSTED_FW) && !defined(S64)
...
void thread_init_per_cpu(void)
{
	size_t pos = get_core_pos(); // returns 0 bc one thread
	struct thread_core_local *l = thread_get_core_local();
	set_tmp_stack(l, GET_STACK(stack_tmp[pos]) - STACK_TMP_OFFS);
	set_abt_stack(l, GET_STACK(stack_abt[pos]));
	init_tss(l);
