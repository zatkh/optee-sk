...
#ifdef X64
	// TODO: Why are these necessary for x64?
...
uint32_t __thread_enter_user_mode(struct tee_usr_args* args,
		unsigned long user_sp, unsigned long user_func,	uint32_t *exit_status0,
		uint32_t *exit_status1);
#endif
 ...
 *
 * TODO: This must  be dealt with cleanly for x64.
 */
int thread_state_suspend(uint32_t flags,
#if defined(ARM32) || defined(ARM64)
						 uint32_t cpsr,
#endif
						 vaddr_t pc);
